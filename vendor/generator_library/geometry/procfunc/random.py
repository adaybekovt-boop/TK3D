...
@pf.tracer.primitive(allow_exec=True)
def normal(rng: np.random.Generator, mean: float, std: float) -> float:
...
@pf.tracer.primitive(allow_exec=True)
def uniform_tails(
    rng: np.random.Generator, low: float, high: float, tail_pct: float = 0.05
) -> float:
    if rng.random() < 0.5:
...
@pf.tracer.primitive(allow_exec=True)
def exponential(rng: np.random.Generator, scale: float) -> float:
...
@pf.tracer.primitive(allow_exec=True)
def poisson(rng: np.random.Generator, lam: float) -> float:
    return rng.poisson(lam)


@pf.tracer.primitive(allow_exec=True)
def triangular(
    rng: np.random.Generator, low: float, high: float, mode: float
) -> float:
    return rng.triangular(low, mode, high)


@pf.tracer.primitive(allow_exec=True)
def gamma(rng: np.random.Generator, shape: float, scale: float) -> float:
    return rng.gamma(shape, scale)


@pf.tracer.primitive(allow_exec=True)
def binomial(rng: np.random.Generator, n: int, p: float) -> float:
    return rng.binomial(n, p)


@pf.tracer.primitive(allow_exec=True)
def geometric(rng: np.random.Generator, p: float) -> float:
    return rng.geometric(p)


random_distrib_funcs = (
    uniform, normal, uniform_tails, clip_gaussian, wrap_gaussian,
    log_uniform, log_normal, spherical_sample, mixture_of_gaussian,
    beta, poisson, triangular, gamma, binomial, geometric, exponential,
)
