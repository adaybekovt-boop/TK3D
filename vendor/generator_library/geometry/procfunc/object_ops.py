...
def set_material(
    ...
    if material is None:
        if all(x is None for x in [material, surface, displacement, volume]):
            raise ValueError(
...
@pf.tracer.primitive(mutates=["mutates_obj"])
def shade_flat(mutates_obj: t.MeshObject, keep_sharp_edges: bool = True) -> None:
    execute_object_op(
...
def join(mutates_obj_1: t.MeshObject, mutates_obj_2: t.MeshObject) -> None:
    execute_object_op(
        bpy.ops.object.join,
...
@pf.tracer.primitive(mutates=["mutates_obj"])
def shade_smooth(mutates_obj: t.MeshObject, keep_sharp_edges: bool = True) -> None:
    execute_object_op(
...
@pf.tracer.primitive()
def mesh_to_curve(mesh: t.MeshObject) -> t.CurveObject:
    execute_object_op(
        bpy.ops.object.convert,
        active=mesh,
        target="CURVE",
