...
def create_stairs(bm, faces, prop):
    ...
    for f in faces:
        ...
        if not valid_ngon(f):
            popup_message(
                "Stairs creation not supported for non-rectangular n-gon!", "Ngon Error"
            )
            return False
...
def create_filled_steps(bm, face, step_widths, step_height):
    ...
    for i, step_width in enumerate(step_widths):
        if i == 0:
            ...
            top_faces.append(
                [f for f in surrounding_faces if vec_equal(f.normal, VEC_UP)][0]
            )
        else:
            bottom_face = list(
                {
                    f
                    for e in front_face.edges
                    for f in e.link_faces
                    if vec_equal(f.normal, VEC_DOWN)
                }
            )[0]
            top_face, front_face, _ = extrude_step(
                bm, bottom_face, normal, step_height, step_width
            )
...
def create_blocked_steps(bm, face, step_widths, step_height):
    ...
    for i, step_width in enumerate(step_widths):
        if i == 0:
            ...
            bottom_face = sort_faces(
                list({f for e in inner_edges for f in e.link_faces}), normal
            )[1]
    ...
    for i, step_width in enumerate(step_widths):
        if i == 0:
            ...
            bmesh.ops.remove_doubles(
                bm, verts=list(e1.verts) + list(e2.verts), dist=0.001
            )
...
def extrude_step(bm, face, normal, step_height, step_width):
    ...
    front_face = list(
        {f for e in face.edges for f in e.link_faces if vec_equal(f.normal, normal)}
    )[0]
    front_face, surrounding_faces = extrude_face(bm, front_face, step_width)
    flat_edges = list(
        {
...
def subdivide_next_step(bm, ret_face, remaining, step_height):
    ...
    return subdivide_face_vertically(
        bm, ret_face, widths=[remaining * step_height, step_height]
    )[0]
...
def create_stairs_split(bm, face, prop):
    ...
    f = create_face(
        bm,
        size,
        prop.size_offset.offset
        - Vector((0, (h_height) - prop.step_height * (prop.step_count + 0.5))),
        xyz,
    )
    bmesh.ops.translate(
        bm,
        verts=f.verts,
        vec=face.calc_center_bounds() - face.normal * prop.depth_offset,
    )
...
def add_railing_to_stairs(bm, top_faces, normal, prop):
    ...
    if prop.landing:
        v1, v2 = railing_verts(
            bm, sort_verts(first_step.verts, normal)[:2], normal, offset, corner_pw / 2
        )
        v3, v4 = railing_verts(
            bm,
            sort_verts(first_step.verts, normal)[-2:],
            normal,
            offset,
            -corner_pw / 2,
        )
        v5, v6 = railing_verts(
            bm,
            sort_verts(last_step.verts, normal)[:2],
            normal,
            offset,
            prop.step_width - corner_pw / 2,
        )
    ...
    bmesh.ops.translate(
        bm, verts=top_verts, vec=Vector((0.0, 0.0, 1.0)) * prop.rail.corner_post_height
    )
    ...
    if prop.rail.fill == "WALL" and not prop.rail.bottom_rail:
        for wall in fill:
            ...
            if sloped_edges:
                ...
                bmesh.ops.translate(
                    bm,
                    verts=[v for e in bottom for v in e.verts],
                    vec=(0, 0, -prop.step_height),
                )
