...
class ArchProperty(bpy.types.PropertyGroup):
    """Convenience PropertyGroup to create arched features"""
    ...
    resolution: IntProperty(
        name="Arc Resolution", min=1, max=128, default=6,
        description="Number of segments for the arc",
    )
    depth: FloatProperty(
        name="Arc Depth", min=get_scaled_unit(0.01), max=get_scaled_unit(1.0),
        default=get_scaled_unit(0.05), unit="LENGTH",
        description="How far arc is from top",
    )
    height: FloatProperty(
        name="Arc Height", get=get_height, set=set_height, unit="LENGTH",
        description="Radius of the arc",
    )

    func_items = [("SINE", "Sine", "", 0), ("SPHERE", "Sphere", "", 1)]
    function: EnumProperty(
        name="Offset Function", items=func_items, default="SPHERE",
...
def create_arch(bm, top_edges, frame_faces, arch_prop, frame_thickness, xyz):
    ...
    lower_arc = filter_geom(
        arc_edge(
            bm, arc_edges[1], resolution, height - frame_thickness, xyz, function
        )["geom_split"],
        BMEdge,
    )
    ...
    if len(verts) == 4:
        verts = sort_verts([v for e in top_edges for v in e.verts], xyz[0])
        new_edge = bmesh.ops.connect_verts(
            bm, verts=[verts[1], verts[-2]]
        )["edges"].pop()
