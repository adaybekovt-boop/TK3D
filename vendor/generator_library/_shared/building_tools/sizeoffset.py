...
from ..utils import clamp, restricted_size, restricted_offset
from mathutils import Vector


class SizeOffsetProperty(bpy.types.PropertyGroup):
    """Convenience PropertyGroup used for regular Quad Inset."""
    ...
    size: FloatVectorProperty(
        name="Size", subtype="XYZ", size=2, unit="LENGTH", description="Size of geometry"
    )
    size_width: FloatProperty(
        name="Width", set=set_size_width, get=get_size_width, unit="LENGTH"
    )
    size_height: FloatProperty(
        name="Height", set=set_size_height, get=get_size_height, unit="LENGTH"
    )
    ...
    offset: FloatVectorProperty(
        name="Offset", subtype="TRANSLATION", size=2, unit="LENGTH"
    )
    offset_horizontal: FloatProperty(
        name="Horizontal", set=set_offset_horizontal, get=get_offset_horizontal, unit="LENGTH"
    )
    offset_vertical: FloatProperty(
        name="Vertical", set=set_offset_vertical, get=get_offset_vertical, unit="LENGTH"
    )
    show_props: BoolProperty(default=False)

    def init(
        self, parent_dimensions, default_size=(1.0, 1.0),
        default_offset=(0.0, 0.0), restricted=True, spread=None
    ):
