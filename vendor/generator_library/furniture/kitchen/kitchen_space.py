...
def geometry_nodes_add_cabinet_top(nw: NodeWrangler):
    ...
    generatetabletop = nw.new_node(
        ...
        input_kwargs={
            "Thickness": value, "N-gon": 4, "Profile Width": multiply,
            "Aspect Ratio": divide, "Fillet Ratio": 0.0100,
    ...
    join_geometry = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={"Geometry": [group_input.outputs["Geometry"], transform_geometry]},
    )
    nw.new_node(
        Nodes.GroupOutput,
    ...
    def __init__(self, factory_seed, coarse=False, dimensions=None, island=False):
        ...
        with FixedSeed(factory_seed):
            if dimensions is None:
                dimensions = Vector((uniform(0.7, 1), uniform(1.7, 5), uniform(2.3, 2.5)))
            self.island = island
            if self.island:
    ...
    def create_asset(self, **params):
        ...
        if not self.island:
            ...
            cabinet_top_right.location = (
                -x / 4.0, y - cabinet_top_width, z - cabinet_top_height
            )
...
class KitchenIslandFactory(KitchenSpaceFactory):
    def __init__(self, factory_seed):
        super(KitchenIslandFactory, self).__init__(factory_seed=factory_seed, island=True)
